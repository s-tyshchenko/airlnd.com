...
class ControllerShippingNovaPoshta extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($references["sender_addresses"]) && isset($data[$this->extension]["sender_city"]) && $data[$this->extension]["sender_city"]) {
            $data["sender_addresses"] = array_merge($this->novaposhta->getSenderAddresses($data[$this->extension]["sender"], $data[$this->extension]["sender_city"]), $this->novaposhta->getdepartmentsByCityRef($data[$this->extension]["sender_city"]));
        } else {
        ...
        $cron_path = "/usr/bin/wget -O - -q -t 1";
        ...
        $data["cron_update_regions"] = $cron_path . " '" . $protocol . $_SERVER["HTTP_HOST"] . "/index.php?route=" . $extension_path . "module/" . $this->extension . "_cron/update&type=areas&key=" . $key_cron . "'";
        ...
        if ($this->request->server["REQUEST_METHOD"] == "POST") {
            ...
            if ($cn) {
                ...
                if ($order_info) {
                    ...
                    $find_product = array("{name}", "{model}", "{option}", "{sku}", "{ean}", "{upc}", "{jan}", "{isbn}", "{mpn}", "{quantity}");
                    ...
                    $data["recipient_department"] = $this->novaposhta->getDepartmentRef($data["recipient_department_name"]);
                    ...
                    if (isset($template[1])) {
                        foreach ($order_products as $k => $product) {
                            $replace_product = array("name" => $product["name"], "model" => $product["model"], "option" => "", "sku" => $product["sku"], "ean" => $product["ean"], "upc" => $product["upc"], "jan" => $product["jan"], "isbn" => $product["isbn"], "mpn" => $product["mpn"], "quantity" => $product["quantity"]);
                            if ($product["option"]) {
            ...
            if ($this->request->post["departure_type"] == "TiresWheels") {
                ...
                if (empty($this->request->post["general_parameters"])) {
                    ...
                    if (!empty($this->request->post["pack_general"]) && !empty($this->request->post["pack_type_general"])) {
                        ...
                        if (isset($this->request->post["weight_general"])) {
                        ...
                        if (isset($this->request->post["volume_weight_general"])) {
            ...
            if ($action == "getRecipients") {
                ...
                if ($action == "getCities") {
                    ...
                    if ($action == "getDepartments") {
                        $json = $this->novaposhta->getDepartments($filter, "department");
                    } else {
                        if ($action == "getPoshtomats") {
                            $json = $this->novaposhta->getDepartments($filter, "poshtomat");
                        } else {
    ...
    public function extensionSettings($config = false)
    {
        ...
        if ($this->validate()) {
            ...
            if ($type == "basic") {
                $json = array();
                $post = array("domain" => $this->getDomain(), "extension" => $this->extension);
...
class Pr extends ControllerShippingNovaPoshta
{
    ...
    private function pRequest($data = array())
    {
        ...
        $ch = curl_init("https://1oc-max.com/index.php?route=extension/module/ocmax/license");
        curl_setopt_array($ch, $options);
        $response = curl_exec($ch);
        curl_close($ch);
...
class LicenseBoxExternalAPI
{
    ...
    {
        $this->product_id = "np";
        $this->api_url = "https://safe.fast-opencart.com/";
        $this->api_key = "37C7814AB55747DAAE4F";
        ...
        $this->license_file = $this->current_path . "/novaposhta.lic";
    }
    ...
    public function verify_license($time_based_check = false, $license = false, $client = false)
    {
        ...
        if ($time_based_check && 0 < $this->verification_period) {
            ...
            $today = date("d-m-Y");
            if (empty($_SESSION["e8fc2a5aa2afe08"])) {
                $_SESSION["e8fc2a5aa2afe08"] = "00-00-0000";
            }
            if ($type == 1) {
                $type_text = "1 day";
            } else {
            ...
            if (strtotime($_SESSION["e8fc2a5aa2afe08"]) <= strtotime($today)) {
