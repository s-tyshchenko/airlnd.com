          ...
          <fieldset>
            <legend>
              ...
              <div class="input-group">
                <input class="form-control input-sm" id="add-content-position-name">
                <div class="input-group-btn">
                  <button type="button" onclick="addContentPosition($('#add-content-position-name').val());" data-toggle="tooltip" title="<?php echo $button_module_add; ?>" class="btn btn-primary btn-sm"><i class="fa fa-plus-circle"></i></button>
                </div>
              </div>
            </legend>
            ...
            <?php
              $grouped = array_reduce($layout_modules, function ($result, $item) {
                  $key = $item['position'];
                  if (!isset($result[$key])) {
                      $result[$key] = [];
                  }
            ...
            <div class="row" id="grouped-layout-modules-container">
              <?php foreach ($grouped as $position => $layout_modules) { ?>
                <div class="col-lg-3 col-md-4 col-sm-12">
                <table id="module-<?php echo $position; ?>" class="table table-striped table-bordered table-hover">
                  <thead>
                  <tr>
                    <td class="text-center"><?php echo ucwords(str_replace('_', ' ', $position)); ?></td>
                  </tr>
                  </thead>
                  <tbody>
                  ...
                  <tr>
                    <td class="text-left"><div class="input-group">
                        ...
                        <div class="input-group-btn">
                          <button type="button" onclick="addModule('<?php echo $position; ?>');" data-toggle="tooltip" title="<?php echo $button_module_add; ?>" class="btn btn-primary btn-sm"><i class="fa fa-plus-circle"></i></button>
                        </div>
                      </div></td>
                  </tr>
                  </tfoot>
...
$('#module-column-left, #module-column-right, #module-content-top, #module-content-bottom').trigger('change');
  ...
  <script>
    function addContentPosition(name) {
      html = '<div class="col-lg-3 col-md-4 col-sm-12">' +
      '<table id="module-' + name + '" class="table table-striped table-bordered table-hover"><thead><tr>' +
      '<td class="text-center">' + name + '</td>' +
      '</tr></thead><tbody></tbody><tfoot><tr>' +
      '<td class="text-left"><div class="input-group"><select class="form-control input-sm">';
      ...
      html += '</optgroup>'
      <?php } ?>
      html += '</select><div class="input-group-btn"><button type="button" onclick="addModule(\'' + name + '\');" data-toggle="tooltip" title="<?php echo $button_module_add; ?>" class="btn btn-primary btn-sm"><i class="fa fa-plus-circle"></i></button></div></div></td></tr></tfoot>';

      $('#grouped-layout-modules-container').append(html);
    }
  </script>
</div>
<?php echo $footer; ?>