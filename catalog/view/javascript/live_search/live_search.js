const LiveSearchJs = function () {
    ...
    const init = function (options) {
        initOnce(options)
        initOnce(options, '_mobile')

        $(document).bind("mouseup touchend", function (e) {
            var containers = $('.live-search, .live-search--mobile');
    ...
    const initOnce = function (options, suffix = '') {
        const live_search = {
            selector: `#search${suffix} input[name='search']`,
            text_no_matches: options.text_empty,
            text_sku: options.text_sku,
            height: '50px'
        }

        $(live_search.selector).after(`<div class="live-search${suffix}"><ul class="live-search--container"></ul><div class="result-text${suffix}"></div></div>`);
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            ...
                            if (!$.isEmptyObject(products)) {
                                ...
                                $(`.result-tex${suffix}t`).html('<a href="' + all_search_href + '" class="live-search--btn-all">' + options.text_view_all_results + ' (' + result.total + ')</a>');
                                ...
                                $.each(products, function (index, product) {
                                    ...
                                    html += '<small class="text-muted">' + live_search.text_sku + ': ' + product.sku + '</small>';
                                    html += '</div>';
                                    ...
                                    if (show_add_button == "1") {
                                        ...
                                        html += '<button class="btn btn-outline-secondary" data-toggle="add-to-cart" data-product-id="' + product.product_id + '">';
