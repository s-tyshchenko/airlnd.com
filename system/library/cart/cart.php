...
class Cart {
	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				// Mnufacturer
				$product_manufacturer_query = $this->db->query("SELECT name FROM " . DB_PREFIX . "manufacturer_description WHERE manufacturer_id = '" . (int)$product_query->row['manufacturer_id'] . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1");
				
				if (!empty($product_manufacturer_query->row['name'])) {
					$manufacturer = $product_manufacturer_query->row['name'];
				} else {
					$manufacturer = '';
				}
				...
				if($price !== $price_old){
					...
					$total_old = $price_old + $option_price;
				} else {
					$price_old = false;
					$total_old = false;
				}
				...
				$product_data[] = array(
					...
					'total_old'       => $total_old * $cart['quantity'],
	...
	public function hasStock() {
	    return true; // allow order creation even when products are out of stock
